...
class get_model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.args.activ(self.conv1(x))
        x1 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x1, k=self.n_knn)
        x = self.args.activ(self.conv2(x))
        x2 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x2, k=self.n_knn)
        x = self.args.activ(self.conv3(x))
        x3 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x3, k=self.n_knn)
        x = self.args.activ(self.conv4(x))
        x4 = x.max(dim=-1, keepdim=False)[0]

        x = torch.cat((x1, x2, x3, x4), dim=1)

        x = self.args.activ(self.conv5(x))
        x1 = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)
        x2 = F.adaptive_avg_pool1d(x, 1).view(batch_size, -1)
        x = torch.cat((x1, x2), 1)

        x = self.args.activ(self.bn6(self.linear1(x)))
        x = self.dp1(x)
        x = self.args.activ(self.bn7(self.linear2(x)))
        x = self.dp2(x)
        x = self.linear3(x)
